...
use windows::Win32::System::Threading::{CreateEventA, CreateMutexA, EVENT_ALL_ACCESS, OpenEventA, PulseEvent};
    ...
    pub fn new() -> Self {
        ...
        mutexes.push_back(unsafe {
            CreateMutexA(
                None,
                true,
                constants::SYNAPSE_ONLINE_MUTEX,
            )
                .unwrap()
        });
        ...
        mutexes.push_back(unsafe {
            CreateMutexA(
                None,
                true,
                constants::OLD_SYNAPSE_ONLINE_MUTEX,
            )
                .unwrap()
        });
        Self::pulse_event(constants::SYNAPSE_EVENT);
        mutexes.push_back(unsafe {
            CreateMutexA(
                None,
                true,
                constants::OLD_SYNAPSE_VERSION_MUTEX,
            )
                .unwrap()
        });
        Self::pulse_event(constants::SYNAPSE_EVENT);
        mutexes.push_back(unsafe {
            CreateMutexA(
                None,
                true,
                constants::CHROMA_EMULATOR_MUTEX,
            )
                .unwrap()
        });
